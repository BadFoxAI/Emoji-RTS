...
/**
 * Handles right-click events on the game viewport for context actions.
 * Implements "smart" right-click for common default actions.
 */
function gameHandleContextMenu(event) {
    ...
    if (selectedUnit && selectedUnit.faction === playerFactionKey) {
        ...
        if (clickedOnGameObject) {
            ...
            if ((targetUnitData && targetUnitData.faction !== playerFactionKey) || 
                (targetBuildingData && targetBuildingData.faction !== playerFactionKey && !targetBuildingData.isConstructing) || 
                (targetConstructionData && targetConstructionData.faction !== playerFactionKey) 
               ) {
                if (selectedUnit.attackDamage > 0) {
                    const actualTargetElement = targetUnitData?.element || targetBuildingData?.element || targetConstructionData?.element;
                    const targetBox = getElementWorldBoundingBox(actualTargetElement);
                    issueCommand(selectedUnit, { 
            ...
            // PRIORITY 2: WORKER ACTIONS
            else if (selectedUnit.unitType === 'worker' && !commandIssued) { 
                if (targetResourceData && !(targetResourceData.type === 'mine' && targetResourceData.health <= 0)) {
                    ...
                    issueCommand(selectedUnit, { 
                        ...
                        targetElement: targetResourceData.element, 
                        target: { x: targetBox.centerX, y: targetBox.centerY }, 
                        preferredType: targetResourceData.type 
                    });
                    commandIssued = true;
                } else if (targetConstructionData && targetConstructionData.faction === playerFactionKey && selectedUnit.canBuild && 
                           (!targetConstructionData.assignedWorker || targetConstructionData.assignedWorker === selectedUnit)) {
                    ...
                    commandIssued = true; 
                     ...
                     issueCommand(selectedUnit, { 
                         state: 'returning', 
                         targetElement: targetBuildingData.element, 
                         target: { x: targetBox.centerX, y: targetBox.centerY } 
                        });
                     commandIssued = true;
                }
            }
             // If still no command issued, and it's a clickable friendly/neutral non-resource object, could open context menu
             if (!commandIssued && ( (targetUnitData && targetUnitData.faction === playerFactionKey) || 
                                     (targetBuildingData && targetBuildingData.faction === playerFactionKey ) ) ) {
                // Example: Could open a menu to "Follow" or "Guard" friendly unit, or "Set Rally Point" for building
                // For now, we'll let it fall through to move if no specific menu is defined.
                // builderFn = (ctxData) => buildFriendlyInteractionMenu(ctxData, targetUnitData || targetBuildingData);
             }
        }

        if (!commandIssued && !builderFn) { // If no direct command or specific menu, default to move.
    ...
    if (builderFn && contextMenu) { // Only show context menu if a builder function was set
...
function handleGlobalKeyDown(e) {
    ...
    if (gameMode === 'human_vs_ai' && !e.target.matches('input, textarea')) {
        ...
        if (key === 'escape') {
            ...
            if (commandCardMenu && commandCardMenu.isVisible && commandCardMenu.menuStateStack.length > 0) { /* JSRTSMenu's Esc handles its submenus */ return;}
            else if (commandCardMenu && commandCardMenu.isVisible && commandCardMenu.menuStateStack.length === 0 && !commandCardMenu.searchQuery){ commandCardMenu.hide(); e.preventDefault(); return;} 
...
function calculateCurrentFood(factionKeyToCalc) { 
    // Ensure FACTION_DATA and units are available (they are global in this setup)
    if (!FACTION_DATA[factionKeyToCalc] || !FACTION_DATA[factionKeyToCalc].units || !units) return 0;
...
function calculateFoodCapacity(factionKeyToCalc) { 
    let capacity = 0; 
    if (!FACTION_DATA[factionKeyToCalc] || !FACTION_DATA[factionKeyToCalc].buildings || !buildings) return STARTING_FOOD_CAP; // STARTING_FOOD_CAP from game-data.js
    buildings.forEach(b => { 
